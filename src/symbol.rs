...
macro_rules! intern_static {
    ($mod:ident, $mod_doc:expr, $name:ident => {$($enum_ident:ident; $static_ident:ident: $str:expr; $doc:expr),*$(,)?}) => {
        ...
        pub mod $mod {
            $(
                #[doc = $doc]
                pub static $static_ident: ::once_cell::sync::Lazy<$crate::symbol::Symbol>
                    = ::once_cell::sync::Lazy::new(|| $crate::symbol::Symbol::intern_static_2($str));
            )*

            /// Initialize all of the symbols in this module.
            pub(super) fn init() {
                $(::once_cell::sync::Lazy::force(&$static_ident);)*
            }
